...
public class Histogram extends HistogramBase {
    ...
    public int noiseMultiplier(int bucketIdx) {
        // Cast down - the noise operation is only valid on the dyadic histogram leaves
        DyadicHistogramBuckets db = (DyadicHistogramBuckets)bucketDescription;

        int nLeaves = db.numLeavesInBucket(bucketIdx);
    ...
    public void addDyadicLaplaceNoise(double scale) {
        ...
        for ( int i = 0; i < this.buckets.length; i++ ) {
            this.buckets[i] += dist.sample() * noiseMultiplier(i);
        }
    }
