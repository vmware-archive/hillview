...
/**
 * Knows how to read a CSV file (comma-separated file) compressed as gz file.
 */
public class CsvGzReader extends TextFileReader {
    ...
    private final CsvGzReader.CsvConfiguration configuration;
    @Nullable
    private Schema actualSchema;

    public CsvGzReader(final Path path, CsvGzReader.CsvConfiguration configuration) {
        super(path);
    ...
    public ITable read() throws IOException {
        ...
        try (Reader file = new InputStreamReader(new GZIPInputStream(new FileInputStream(this.filename.toString())))) {
