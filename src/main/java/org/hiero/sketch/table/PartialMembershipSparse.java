 ...
 *
 * This implementation uses a Set data structure to store the membership. It uses the Set's membership and iterator methods. The upside is that it is efficient in space and that the iterator is very efficient. So this implementation is best when the set is sparse.
...
public class PartialMembershipSparse implements IMembershipSet {
    ...
    /* Standard way to construct this map is by supplying a memshipSet (perhaps the full one), and the filter function passed as a lambda expression*/

    public PartialMembershipSparse(IMembershipSet baseMap, Predicate<Integer> filter) throws NullArgumentException {
