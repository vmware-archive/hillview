...
public class Histogram1DTest {
    ...
    public void testHistogram1D() throws Exception {
        ...
        assertTrue(size + hist.getMissingData() + hist.getOutOfRange() == 10000);
        ...
        assertTrue(size + hist1.getMissingData() + hist1.getOutOfRange() == 20000);
        Histogram1D hist2 = hist1.union(hist);
        size = 0;
        for (int i = 0; i < bucketnum; i++)
            size += hist2.getBucket(i).getCount();
        assertTrue(size + hist2.getMissingData() + hist2.getOutOfRange() == 30000);
