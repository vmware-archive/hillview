...
import {HeatMapData, AxisData} from "./heatmap";
...
export class LegendOverlay implements IHtmlElement {
    ...
    public update(pos: Point2D, val: number) {
...
export class HeatMapArrayView extends RemoteObjectView implements IScrollTarget {
    ...
    private numHeatMaps() {
        return this.args.uniqueStrings.length;
    }
    ...
    private initiateRange2D() {
        ...
        rr.invoke(
            new OnCompleteRenderer<Pair<BasicColStats, BasicColStats>>(this.getPage(), rr, (stats) => {
                this.setStats(stats);
                this.legendOverlay = new LegendOverlay(this.getPage(), this.args.cds, this.args.xStats, this.args.yStats);
    ...
    private initiateHeatMaps() {
        ...
        rr.invoke(
            new OnCompleteRenderer<HeatMapArrayData>(this.getPage(), rr, (heatMapArray) => this.updateView(heatMapArray), "Heat Map Array")
        );
    }
